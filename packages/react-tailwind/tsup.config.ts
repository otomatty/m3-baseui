...
/**
 * Multi-entry build so every subpath (`@m3-baseui/react-tailwind/button`, …) maps to
 * its own dist file. Each entry calls a client `create*` factory from @m3-baseui/core
 * at module init, so it must itself be a client module: in Next/RSC a server
 * component importing `@m3-baseui/react-tailwind/button` would otherwise execute that
