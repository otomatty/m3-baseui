 ...
 * pointer ripple is added by the factory. Per M3 Expressive the selection color
 * transition rides the effects spring and the checkmark's width (a spatial change)
 * the spatial spring. Same DOM as the VE build.
 */
import { createSegmentedButton } from '@m3-baseui/core';
import { tv } from '../../tv';

// Color changes use the fast effects spring; the checkmark's width (a spatial
// change) uses the fast spatial spring. Tailwind exposes the spring easings but
// not their durations, so durations come from the raw custom properties.
const SPRING_COLOR =
  'transition-colors duration-[var(--md-sys-motion-duration-spring-effects-fast)] ease-spring-effects-fast';
const SPRING_CHECK =
  'transition-[width,opacity] duration-[var(--md-sys-motion-duration-spring-spatial-fast)] ease-spring-spatial-fast';
